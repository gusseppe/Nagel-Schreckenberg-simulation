...
class Road:
    ...
    def inBounds(self, pos):
        """ Todo carro debe estar dentro de los limites permitidos \
                el x maximo es la longitud de la pista, \
                el y maximo es el ultimo carril """

        return pos[0] >= 0 and pos[1] >= 0 and pos[0] < self.getLength() and pos[1] < self.getLanesCount()

    def clearAt(self, pos):
        """ Limpiar una posicion de la pista, es decir, borrar una carro  """

        self.lanes[pos[1]][pos[0]] = None

    def placeObjects(self, entities):
        """ Ubicar todo los carros en la pista """

        return all(self.placeObject(entity) for entity in entities)

    def placeObject(self, entity):
        """ Ubicar el carro en la pista, cada carro tiene su posicion \
                posicion en X y posicion en Y (numero de carril) """
    ...
    def getLanesCount(self):
        """ Calcular la cantidad de carriles """

        return len(self.lanes)

    def getCellCount(self):
        """ La longitud de la celda es la longitud de la pista\
                por la cantidad de carriles """

        return self.getLength() * self.getLanesCount()
        
    def getAvgCarSpeed(self):
        """ Iterar a traves de todos los carros y sumar sus velocidades, \
        luego dividir entre el numero de carros """
    ...
    def generateEmptyLanes(lanesCount, length):
        """ Generar la pista vacia, con todos su carriles"""
